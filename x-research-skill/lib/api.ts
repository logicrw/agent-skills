...
function readEnvToken(path: string | URL): string | undefined {
  try {
...
function getToken(): string {
  ...
  const fromCwd = readEnvToken(`${process.cwd()}/.env`);
  if (fromCwd) return fromCwd;

  const fromSkill = readEnvToken(new URL("../.env", import.meta.url));
